...
fix: ## Fix all lockfiles and add any missing revisions
	...
	find ./$(TOOLSET) -name '*.yml' ! -path .//.schema.yml | grep '^\./[^/]*/' | sort | xargs -P 1 python scripts/update_tool.py --without
...
update-trusted: ## Run the update script for a subset of repos
	...
	find ./$(TOOLSET) -name '*.yml' ! -path .//.schema.yml | grep '^\./[^/]*/' | sort | xargs -P 1 python scripts/update_tool.py --owner $(OWNER)

update-all: ## Run the update script for all repos
	find ./$(TOOLSET) -name '*.yml' ! -path .//.schema.yml | grep '^\./[^/]*/' | sort | xargs -P 1 python scripts/update_tool.py
